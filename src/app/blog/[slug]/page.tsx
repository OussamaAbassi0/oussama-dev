...
export async function generateMetadata({
  ...
  if (!article) return {};
  const lang    = (searchParams.lang ?? "fr") as Lang;
...
/* ── Rendu Markdown simple (sans librairie) ─────────────── */
...
export default function ArticlePage({
  ...
  const lang      = validLangs.includes(searchParams.lang as Lang) ? searchParams.lang as Lang : "fr";
  ...
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", paddingTop:"80px" }}>
      ...
      {/* Nav top */}
      <div style={{ maxWidth:"760px", margin:"0 auto", padding:"24px 24px 0", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"12px" }}>
        ...
        <div style={{ display:"flex", gap:"6px" }}>
          {validLangs.map(l => (
            <Link key={l} href={`/blog/${params.slug}?lang=${l}`} style={{
              padding:        "4px 12px",
      ...
      >
        ...
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"24px", flexWrap:"wrap" }}>
          ...
          <span style={{ fontFamily:"'Courier New',monospace", fontSize:"10px", color:"rgba(255,255,255,.3)" }}>
            {new Date(article.date).toLocaleDateString(lang === "ar" ? "ar-SA" : lang, { year:"numeric", month:"long", day:"numeric" })}
          </span>
          <span style={{ fontFamily:"'Courier New',monospace", fontSize:"10px", color:"rgba(255,255,255,.3)" }}>
            · {article.readTime}
          </span>
        ...
        <div style={{
          ...
          <p style={{
            ...
            {lang==="ar" ? "هل تريد بناء هذا لشركتك؟" : lang==="es" ? "¿Quieres construir esto para tu empresa?" : lang==="en" ? "Want to build this for your business?" : "Vous voulez que je construise ça pour vous ?"}
          </p>
          <p style={{ fontFamily:"Arial,sans-serif", fontSize:"14px", color:"rgba(255,255,255,.5)", marginBottom:"20px" }}>
            {lang==="ar" ? "أخبرني بمشكلتك وسأعود إليك بخطة عمل واضحة خلال 24 ساعة." : lang==="es" ? "Cuéntame tu problema y te respondo con un plan de acción claro en 24h." : lang==="en" ? "Tell me your problem and I'll get back to you with a clear action plan within 24h." : "Décrivez votre problème, je vous réponds avec un plan d'action concret sous 24h."}
          </p>
          <Link href="/#cta" style={{
            ...
            {lang==="ar" ? "ابدأ مشروعك ←" : lang==="es" ? "Iniciar proyecto →" : lang==="en" ? "Start your project →" : "Démarrer mon projet →"}
          </Link>
        </div>
      </article>
    </div>
